...
# SPECIFICATION: association rule
...
for index, row in df.iterrows():
    row_set = set(row)
    labels = {}
    uncommons = list(itemset-row_set)
    commons = list(itemset.intersection(row_set))
    for uncommon_item in uncommons:
        labels[uncommon_item] = 0
    for common_item in commons:
        labels[common_item] = 1
...
for idx, row in rules.iterrows():
    ...
    print(f"{actecident[:-2]} -> {consequent[:-2]}")
    ...
    print(f"Prior: {prior}")
    rule_confidence = 100 * (row["antecedent support"] - prior) / prior
    print(f"Gain in Confidence: {rule_confidence}")
    print()
